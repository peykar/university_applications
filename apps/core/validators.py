...
def parse_phone_number(value: str, region: str | None = None) -> phonenumbers.PhoneNumber:
    """Parse and validate a phone number.

    If no region is supplied, the number must be in international form (for
    example +31612345678). Numbers starting with 00 are accepted and converted
    to + before parsing.
    """
    value = _prepare_phone_value(value)

    if not value:
        raise ValidationError(_("Enter a phone number."), code="invalid_phone")
