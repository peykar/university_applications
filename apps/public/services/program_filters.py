...
def apply_program_filters(
    ...
    if state.q:
        queryset = queryset.filter(
            ...
            | Q(general_fields__name_en__icontains=state.q)
            | Q(general_fields__name_fa__icontains=state.q)
            | Q(general_fields__name_tr__icontains=state.q)
            | Q(general_fields__name_ar__icontains=state.q)
        )

    if state.field:
        queryset = queryset.filter(general_fields__slug_en=state.field)
