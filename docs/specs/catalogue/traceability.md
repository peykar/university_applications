...
| `CAT-050` | `CAT-T35` globally unique Program public slugs | `CAT-T35` | `apps/universities/models.py::Program._populate_missing_slugs`; `apps/universities/management/commands/rebuild_program_slugs.py`; normalized importer Program upsert | `ProgramCanonicalPublicSlugTests` including structured hierarchy, missing-localization fallback, thesis, and multilingual variants; `RebuildProgramSlugsCommandTests` including structured hierarchy fallback, thesis rebuild, and collision report/skip continuation; normalized importer tests |
