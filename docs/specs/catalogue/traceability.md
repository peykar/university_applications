...
| `CAT-043` | Catalogue v3 UI/API consumers | `CAT-T29`, `CAT-T36`, `CAT-T37`, `CAT-T39` | `apps/public/services/program_filters.py`, `apps/public/views.py`, public/customer/agent templates including customer-facing “Tuition from” Similar-program pricing copy, Application admin, ProgramOffering API serializer | `tests/test_catalogue_v3_ui.py` including rendered structured-fee presentation, `tests/test_program_filters.py`, `tests/test_similar_program_cards_clickable.py`, customer Request structural tests |
...
| `CAT-050` | `CAT-T35` globally unique Program public slugs | `CAT-T35` | `apps/universities/models.py::Program._populate_missing_slugs`; `apps/universities/management/commands/rebuild_program_slugs.py`; normalized importer Program upsert | `ProgramCanonicalPublicSlugTests` including structured hierarchy, missing-localization fallback, thesis, and multilingual variants; `RebuildProgramSlugsCommandTests` including structured hierarchy fallback, thesis rebuild, and deterministic numeric collision-tail resolution; normalized importer tests including ordinary-save localized collision tails and idempotent re-import |
| `CAT-051` | `CAT-T38` persisted Catalogue v3 audit | `apps/universities/management/commands/audit_catalogue.py`; `docs/catalogue-audit.md` | `tests/test_catalogue_audit.py` |
| `CAT-052` | Stable locale-independent public filter identity; Department-backed field classification superseded by `CAT-055` | `CAT-T40`, `CAT-T43` | public filters continue submitting canonical `slug_en`; CHG-0017 resolves field identity through GeneralField | `tests/test_program_filters.py` canonical slug identity/localized label regression |

| `CAT-053` | Global multilingual GeneralField | `CAT-T41`, `CAT-T45` | `apps/universities/models.py::GeneralField` | model/admin coverage plus SDD validation |
| `CAT-054` | Optional Program GeneralField mapping | `CAT-T41` | `apps/universities/models.py::Program.general_fields` | `tests/test_university_program_json_import.py`; `tests/test_program_filters.py` |
| `CAT-055` | GeneralField-backed public field filtering | `CAT-T43`, `CAT-T46` | `apps/public/services/program_filters.py`; `apps/public/views.py::_canonical_field_choices`; homepage study fields | `tests/test_program_filters.py` |
...
| `CAT-067` | Optional City banner, localized alt text and public/admin presentation | `CAT-T57`–`CAT-T60` | `apps/geography/models.py::City`; `apps/geography/admin.py::CityAdmin`; `apps/public/views.py::university_city_detail`; `templates/public/university_city_detail.html`; `static/css/turkdemy.css` | `tests/test_taxonomy_enrichment.py`; `tests/test_public_page_metadata.py` |
