...
- [ ] CAT-T01 (`CAT-007`, `CAT-008`, `CAT-020`) Add AcademicUnit model,
      validation, admin maintenance, and Program association.
- [ ] CAT-T02 (`CAT-009`, `CAT-010`, `CAT-021`) Add
      ProgramInstructionLanguage through model and data-preserving legacy
      language migration.
- [ ] CAT-T03 (`CAT-011`, `CAT-020`, `CAT-022`, `CAT-023`) Add study mode,
      importer mapping, admin display, public presentation/filter support.
- [ ] CAT-T04 (`CAT-012`, `CAT-021`, `CAT-022`, `CAT-023`) Introduce canonical
      fraction-safe duration, migrate existing values, update displays/imports.
- [ ] CAT-T05 (`CAT-014`, `CAT-021`) Rename/migrate `pre_school_fees` to
      `preparatory_tuition` without data loss.
- [ ] CAT-T06 (`CAT-016`, `CAT-017`) Add UniversityCatalogueSource, source file
      handling, same-University validation, and admin maintenance.
- [ ] CAT-T07 (`CAT-013`, `CAT-015`, `CAT-018`) Extend ProgramOffering with
      preparation inclusion, notes, validity, and formal pricing semantics.
- [ ] CAT-T08 (`CAT-019`, `CAT-020`) Redesign catalogue admin/agent forms so all
      approved Program and Offering fields are maintainable and validated.
- [ ] CAT-T09 (`CAT-022`) Update Rasa/import mapping and fixtures; preserve
      ambiguous unsupported source values rather than guessing.
- [ ] CAT-T10 (`CAT-023`) Update public program detail/list/filter consumers and
      any Request/Application program displays to canonical fields.
- [ ] CAT-T11 (`CAT-021`) Add migration/regression tests proving existing
      Program, Offering, Application and LeadProgramInterest references/data are
      preserved.
- [ ] CAT-T12 (`CAT-007`–`CAT-023`) Add named model, form/admin, importer and
      public catalogue tests and update traceability to concrete paths.
- [ ] CAT-T13 (`CAT-024`) Create a separate discovery/spec before implementing
      Admission Requirements or credit-transfer structure.
- [ ] CAT-T14 Update `docs/models.md`, `docs/rasa-mapping.md`,
