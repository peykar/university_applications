...
| `CRQ-091` | `apps/leads/forms.py`, `apps/leads/views.py::apply_program`, `apps/leads/services/program_interests.py` | `tests/test_lead_workflow.py::LeadWorkflowTests::test_new_program_reopens_finalized_request_without_undoing_conversion`, `tests/test_lead_workflow.py::LeadWorkflowTests::test_existing_program_does_not_reopen_finalized_request`, `tests/test_apply_program_selector_ux.py` | Covered |
| `CRQ-092` | `apps/leads/models.py::LeadStatus`, `templates/includes/applicant_entity_header.html`, customer Programs workspace | lifecycle runtime tests + customer workspace structural tests | Covered |
| `CRQ-093` | customer/Agent Lead edit and document mutation guards keyed by `converted_student_id` | `tests/test_finalized_customer_lead_mutations.py`, lifecycle runtime tests | Covered |
| `CRQ-094` | `templates/public/program_detail.html`, `templates/leads/apply_program.html`, `apps/leads/forms.py::ApplyProgramForm`, existing `apps/leads/views.py::apply_program` | `tests/test_discovery_to_request.py`; existing apply/reopen workflow tests | Automated + regression |
